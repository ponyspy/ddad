...
exports.edit_form = function(req, res) {
	...
	async.series({
		desc_ru: function(callback) {
			...
			fs.writeFile(__app_root + '/static/desc_ru.html', 'utf8', post.desc.ru, callback);
		},
		desc_en: function(callback) {
			if (!post.desc.en) return callback(null);

			fs.writeFile(__app_root + '/static/desc_en.html', 'utf8', post.desc.en, callback);
		},
		adress_ru: function(callback) {
			if (!post.adress.ru) return callback(null);

			fs.writeFile(__app_root + '/static/adress_ru.html', 'utf8', post.adress.ru, callback);
		},
		adress_en: function(callback) {
			if (!post.adress.en) return callback(null);

			fs.writeFile(__app_root + '/static/adress_en.html', 'utf8', post.adress.en, callback);
		}
	}, function(err, results) {
		res.redirect('back');
	});
